...
const translations: any = {
  ar: {
    ...
    connectGoogleBtn: "ربط حساب قوقل ماب",
    ...
    noDataDesc: "قم بربط حساب قوقل ماب الخاص بنشاطك التجاري لسحب التقييمات والبدء في الرد عليها آلياً.",
  ...
  en: {
    ...
    connectGoogleBtn: "Connect Google Maps",
    ...
    noDataDesc: "Connect your Google Maps account to fetch reviews and start replying automatically.",
...
const SF_DEMO_GOOGLE_REVIEWS = [
  {
    id: 1, name: 'أحمد عبدالله', avatarLetter: 'أ', stars: 5, time: 'قبل ساعتين', daysAgo: 0,
  ...
  {
    id: 2, name: 'سارة خالد', avatarLetter: 'س', stars: 4, time: 'أمس', daysAgo: 1,
...
const SocialMediaHub = ({
  ...
  const [googleReviewTimeRange, setGoogleReviewTimeRange] = useState('all');
  ...
  const connections = [
    ...
    { provider: 'google', name: 'Google Maps', status: isGoogleConnected ? 'connected' : 'disconnected' },
  ];
  ...
  const timeFilteredGoogleReviews = googleReviewTimeRange === 'today'
    ? googleReviews.filter((r) => r.daysAgo === 0)
    : googleReviewTimeRange === 'week'
    ? googleReviews.filter((r) => r.daysAgo <= 7)
    : googleReviewTimeRange === 'month'
    ? googleReviews.filter((r) => r.daysAgo <= 30)
    : googleReviews;

  const sortedGoogleReviews = googleReviewFilter === 'rating'
    ? timeFilteredGoogleReviews.filter((r) => !r.text)
    : googleReviewFilter === 'comment'
    ? timeFilteredGoogleReviews.filter((r) => !!r.text)
    : googleReviewFilter === 'oldest'
    ? [...timeFilteredGoogleReviews].reverse()
    : timeFilteredGoogleReviews;
                      ...
                      <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'}`}>
                        <div className={`flex flex-wrap items-center justify-between gap-3 p-3 border-b ${isDark ? 'border-slate-800' : 'border-slate-200'}`}>
                          <div className={`flex items-center gap-1 p-1 rounded-xl ${isDark ? 'bg-slate-800/60' : 'bg-slate-100'}`}>
                            ...
                            <span className={`w-px h-4 mx-1 ${isDark ? 'bg-slate-700' : 'bg-slate-300'}`}></span>
                            <select value={googleReviewTimeRange} onChange={(e: any) => setGoogleReviewTimeRange(e.target.value)} className={`px-2 py-1.5 rounded-lg text-xs font-bold outline-none border-none bg-transparent cursor-pointer ${isDark ? 'text-slate-300' : 'text-slate-600'}`}>
          ...
          <div className="space-y-6">
            ...
            {flattenedConversations.length === 0 ? (
              <div className={`flex flex-col items-center justify-center py-16 rounded-3xl border border-dashed ${isDark ? 'bg-slate-900/30 border-slate-800' : 'bg-slate-50 border-slate-300'}`}>
                ...
                <p className="text-slate-500 text-sm max-w-md text-center">اربط حساباتك (WhatsApp، Instagram، TikTok، Google Maps) لاستقبال رسائل العملاء وتعليقاتهم وتقييماتهم والرد عليها جميعاً من مكان واحد.</p>
              </div>
            ) : (
              <div className={`rounded-2xl border overflow-hidden ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'}`}>
